...
export default function ProductsPage() {
  ...
  const loadData = () => {
    fetch('/api/products')
      ...
      .then(data => {
        // 确保rights数组存在
        const productsWithRights = data.map((p: Product) => ({
          ...p,
          rights: p.rights || []
        }))
        setProducts(productsWithRights)
      })
    fetch('/api/rights')
      .then(res => res.json())
  ...
  const handleAdd = async () => {
    const product = await fetch('/api/products', {
      ...
      body: JSON.stringify({
        ...
        duration: formData.type === 'PER_USE' ? null : parseInt(formData.duration),
    ...
    <div className="min-h-screen bg-gray-100">
      <aside className="fixed left-0 top-0 bottom-0 w-64 bg-slate-900 text-white p-4">
        ...
        <nav className="space-y-1">
          ...
          <Link href="/admin/products" className="flex items-center gap-3 px-3 py-2 rounded-lg bg-slate-800 text-white">
      ...
      <main className="ml-64 p-6">
        ...
        {showAdd && (
          <Card className="mb-6">
            <CardHeader>
              <CardTitle className="text-base">新增产品</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="如：365"
                    disabled={formData.type === 'PER_USE'}
                  />
                  {formData.type === 'PER_USE' && (
                    <p className="text-xs text-gray-400 mt-1">次卡无需设置有效期</p>
                  )}
                </div>
                <div className="col-span-2">
                  ...
                  <Input
                    ...
                    placeholder="如：粤运拯救服务1次 + 中石化年卡权益"
                  />
                </div>
                <div className="col-span-2">
                  <Label>关联权益</Label>
                    ...
                    {rights.map(right => (
                      <label
                        ...
                        className={`flex items-center gap-2 px-3 py-1.5 rounded-full border cursor-pointer transition-colors ${
                          formData.selectedRights.includes(right.id)
        ...
        {showEdit && editData && (
          <Card className="mb-6">
            <CardHeader>
              <CardTitle className="text-base">编辑产品</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div>
                ...
                <div>
                  <Label>产品类型</Label>
                  <Input value={getTypeLabel(editData.type)} disabled className="bg-gray-100" />
                ...
                <div>
                  ...
                  <Input
                    ...
                    value={formData.duration}
                    onChange={e => setFormData({ ...formData, duration: e.target.value })}
                    disabled={editData.type === 'PER_USE'}
                  />
                </div>
                <div className="col-span-2">
                  <Label>产品描述</Label>
              ...
              <tbody>
                {products.map((product) => (
                  <tr key={product.id} className="border-b last:border-0">
                    ...
                    <td className="p-4">
                      <Badge variant={product.type === 'YEARLY' ? 'default' : product.type === 'MONTHLY' ? 'secondary' : 'outline'}>
                        {getTypeLabel(product.type)}
                      </Badge>
                    </td>
                    <td className="p-4 font-medium text-blue-600">
                      ¥{product.price}{product.type === 'PER_USE' ? '/次' : ''}
                    </td>
                    <td className="p-4 text-gray-500">
                      {product.duration ? `${product.duration}天` : '-'}
                    </td>
                    <td className="p-4 text-gray-500">
                      {product.rights?.map(r => r.right.name).join('、') || '-'}
                    </td>
                    <td className="p-4">
